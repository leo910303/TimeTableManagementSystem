    ...
    var db2 = {
        ...
        loadData: function(filter) {
            return $.grep(this.clients, function(client) {
                return (!filter.Reg_No || client.Reg_No.indexOf(filter.Reg_No) == 0)
                    && (!filter.Primary_Email || client.Primary_Email.indexOf(filter.Primary_Email) == 0 )
                    && (!filter.Secondary_Email || client.Secondary_Email.indexOf(filter.Secondary_Email) == 0)
                    && (!filter.Faculty || client.Faculty === filter.Faculty)
                    && (!filter.Group || client.Group === filter.Group)
                    && (!filter.Year || client.Year === filter.Year)
                    && (!filter.Mobile || client.Mobile.indexOf(filter.Mobile) == 0);
            });
        },
        ...
        updateItem: function (updatingClient) {
            ...
            var obj=this.clients[clientIndex];
